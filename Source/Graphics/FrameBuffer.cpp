...
FrameBuffer::FrameBuffer(ID3D11Device* device, uint32_t width, uint32_t height)
{
	...
	//オフスクリーン用の深度ステンシル用のテクスチャのシェーダーリソースビュー
...
//オフスクリーン用のレンダーターゲットビューに切り替え(有効化)
void FrameBuffer::activate(ID3D11DeviceContext* immediate_context)
{
	//デバイスコンテキストに現在設定されている
	//ビューポートとレンダーターゲットビューと深度ステンシルビューを一時的に退避(保管)
	...
	//このフレームバッファで作成したビューポートとレンダーターゲットビューと
	//深度ステンシルビューを有効化
